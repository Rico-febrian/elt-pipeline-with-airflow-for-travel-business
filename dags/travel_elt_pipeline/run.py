...
from travel_elt_pipeline.tasks.transform import Transform
...
def travel_elt_pipeline():
    ...
    # Define source and DWH tables to process
    source_tables = [
        "aircrafts_data",
        "airports_data",
        "bookings",
    ...
    def extract():
        ...
        for table_name in source_tables:
            try:
                logger.info(f"Scheduling extraction task for table: {table_name}")
                ...
                logger.error(f"Failed to schedule extraction for {table_name}: {e}", exc_info=True)
                raise
        return tasks
